...
## Описание
&ensp;Проект реализован на парадигме MVP (Model-View-Presenter), в котором слой Model работает с API проекта и данными приложения, слой View - отвечает за отображение данных и взаимодействие с пользователем посредством событий, слой Presenter - отвечает за связь между слоем Model и View, здесь выполнена основная работа с объектами проекта и обработка событий при помощи класса `EventEmitter`.
...
- `options` - объект опций заголовка для формирования секции заголовка запроса API, защищённый;
#### Методы:
...
### Класс `EventEmitter`
&ensp;Брокер событий на основе интерфейса `IEvents`. реализован на основе паттерна «Observer». позволяет устанавливать и снимать слушатели событий и выполнять их обработку
#### Конструктор:
- `constructor()` -инициализирует объект события(`_events`);
#### Типы:
- `EventName` - тип имени события;
- `Subscriber` - тип функции подписчика события;
- `EmitterEvent` - тип объекта события для передачи имени события и объекта данных при обработке события;
#### Поля:
- `_events` - объект типа `Map<EventName, Set<Subscriber>>`, защищённый;
#### Методы:
...
- `onAll` - предназначен для подписки на все события. Входящие параметры: `callback` - объект события с объектом данных обработки;
...
- `order` - данные заказа, заполнение перед отправкой на сервер выполнено поэтапно. Тип данных `Order`;
...
- `wrapperSelector` - элемент разметки с селектором `.page__wrapper` для блокировки содержимого при открытии модального окна;
...
- `category` - set-атрибут для определения свойств отображения для категории. Для элемента разметки `categoryElement` установка класса цвета категории из настройки `SETTINGS['formatView']['categoryColorClass']`;
#### Методы:
...
#### Методы:
- `setButtonEnabled` - установка активности кнопки удаления/добавления продукта на основе входных данных продукта `data: Product`;
...
- `renderError` - вывод текстов ошибок при валидации формы. Принимает объект типа `Errors` со массивом ошибок для полей валидации;
- `render` - возвращает элемент разметки `contentElement` с подготовленными данными. Не принимает входных параметров;
...
## СОБЫТИЯ ПРОЕКТА
&ensp;Список основных событий перечислен в основных настройках проекта в объекте `SETTINGS.appEvents`:
...
- `order:open` - регистрация на событии `onCLick` на кнопке оформления заказа представления класса `Basket`. Обработка события - обновление представления класса `OrderForm`, ;
...
- `contacts:open` - регистрация на событии `submit` на кнопке отправки заказа представления класса `OrderForm`. Обработка события - обновление представления класса `ContactsForm`;
